...
#BACKUP LIMESURVEY FOLDER AND DATABASE FROM SERVER
backup(){
...
#    if [ -z "${1}" ]; then
        ...
        cmd="tar -czf ${backedupFile} ${srcDir}limesurvey/"
        echo "Backing up ${backedupFile} to ${srcDir} : ${cmd} >> ${log}"
        eval ${cmd} >> ${log} 2>&1
...
#    echo "Copying files : ${cmd}"
        ...
        cmd="tar -czf ${backedupDatabase} ${srcDirDB}limesurveybdd/"
        echo "Backing up ${backedupDatabase} to ${srcDir} : ${cmd} >> ${log}"
        eval ${cmd} >> ${log} 2>&1
...
copyToHost(){
    ...
    cmd="scp ${srcDir}${backedupDatabase} ${hostUser}@${host}:${backupDir}"
    echo "Copying ${srcDir}${backedupDatabase} to Host Machine ${backupDir} : ${cmd} >> ${log}"
    eval ${cmd} >> ${log} 2>&1
...
MyFunction='copyFilesToServer'
    ...
    cmd="scp /home/gchris/limesurvey.backups/config/config.php ${serverUser}@${server}:${srcDir}limesurvey/application/config"
...
#COPY CAS PLUGIN FROM HOST TO SERVER
copyCAS(){

MyFunction='copyCAS'

    cmd="scp -r /home/gchris/limesurvey.backups/AuthCAS/ ${serverUser}@${server}:${srcDir}limesurvey/plugins"
    echo "Copying CAS Plugin to Server ${srcDir} : ${cmd} >> ${log}"
    eval ${cmd} >> ${log} 2>&1
...
#CALL SCRIPT copyCAS.sh
callCopyCAS(){

myFunction='callCopyCAS'
script='copyCAS'
scriptPath='/home/gchris/limesurvey.backups/copyCAS.sh'
...
#GIVES PERMISSIONS TO LIMESURVEY FILES
givePerm(){

myFunction='givePerm'

    cmd="chown -R root:www-data ${srcDir}limesurvey/"
    echo "Giving ownership  permissions to Limesurvey : ${cmd} >> ${log}"
    eval ${cmd} >> ${log} 2>&1


    cmd="chmod -R 750 ${srcDir}limesurvey/"
    echo "Giving access permissions to Limesurvey : ${cmd} >> ${log}"
    eval ${cmd} >> ${log} 2>&1


    cmd="chmod -R 770 ${srcDir}limesurvey/tmp/"
    echo "Giving permissions to tmp : ${cmd} >> ${log}"
    eval ${cmd} >> ${log} 2>&1

}


##CALL SCRIPT getUpdate.sh
#callGetUpdate(){
#
#myFunction='callGetUpdate'
#script='getUpdate'
#scriptPath='/home/gchris/limesurvey.backups/getUpdate.sh'
#
##    if [ -z "${1}" ]; then
#
#    cmd="ssh ${hostUser}@${host} '${scriptPath}'"
...
#
#
##DOWNLOAD LIMESURVEY UPDATE AND PLACE IT IN DIRECTORY
#getUpdate(){
#
#myFunction='getUpdate'
#
#    wget=https://www.limesurvey.org/stable-release?download=2159:limesurvey2723%20171020targz
#
#
#}
